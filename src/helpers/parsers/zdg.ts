...
export default async function zdgParse(xmlbody: string) {
  ...
  console.log(`zdg found ${regexpLinks?.length} links`)
  const allLinks = regexpLinks?.filter(
    (link) =>
      link.includes('posttype') && ['2023'].some((year) => link.includes(year))
  )
  if (allLinks) {
    for (let link of allLinks) {
      console.log(`zdg downloading: ${link}`)
    ...
    for (let file of fileArray) {
      console.log(`zdg processing left: ${countDown}`)
      --countDown
      let content = readFileSync(file)
      let parsed = zdgParseChild(content.toString())
...
export function zdgParseChild(xmlbody: string) {
  xmlbody = xmlbody.replaceAll('<loc>', '\r\n')
  xmlbody = xmlbody.replaceAll('\r\n\r\n', '\r\n')
  ...
  merged = new Set(
    [...merged].filter((link: string) => !link.includes('wp-content'))
  )
  return merged
}
